...
def suggestedSongs(id):
    ...
    cursor = coxn.cursor()
    if request.method == 'GET':
        cursor.execute("SELECT TOP 64 ss.SongTitle, ss.Genre, ss.[Length], ss.BPM FROM dbo.Song ss JOIN dbo.Song ps ON ss.Genre = ps.Genre JOIN SongInPlaylist sip ON ps.SongID = sip.SongID WHERE sip.PlaylistID = 2 GROUP BY ss.SongID, ss.SongTitle, ss.Genre, ss.Length, ss.BPM ORDER BY MIN(ABS(ss.BPM - ps.BPM)*ABS(ss.Length - ps.Length)) ASC" if False else "EXEC SimilarSongs ?", id)
        for row in cursor.fetchall():
